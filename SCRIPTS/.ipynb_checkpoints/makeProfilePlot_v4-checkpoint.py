...
def plot_voldens(ax, boundcomp, allcomp, components):
    ...
    for component in components: 
        bound = boundcomp[component]
        allx = allcomp[component]
        bins = None
        cm = bound["cm"]
        if not bound.empty():
            bound_profile = bound.density_profile(
                center=cm,
                bins=bins,
        ...
        all_bins = add_bins(bins, extra_radius.value)
        all_profile = allx.density_profile(
            center=cm,
            bins=all_bins
        )
...
def plot_dispvel(ax, boundcomp, allcomp, components, bins):
    ...
    for component in components: 
        ...
        vcm = bound["vcm"]
        if not bound.empty():
            bound_profile = bound.velocity_profile(
                center=cm,
                v_center=vcm,
        ...
        all_profile = allx.velocity_profile(
            center=cm,
            v_center=vcm,
...
def plot_surfdens(ax,  boundcomp, allcomp, lines_of_sight, components):
    """Adds averaged surface density (both 'all' and 'bound') to plot over multiple lines of sight.
    """
...
def plot_losvel(ax,  boundcomp, allcomp, lines_of_sight, components, bins, velocity_projection):
    """Adds averaged surface density (both 'all' and 'bound') to plot over multiple lines of sight.
    """
    from tqdm import tqdm
...
for _, row in subtree_table.iterrows():
    sn, subtree = row["Snapshot"], row["Sub_tree_id"]

    stars, dm, gas = MT.load_halo(args.particle_data_folder, sub_tree=subtree, snapshot=sn)
    halo_params = MT.get_halo_params(sub_tree=subtree, snapshot=sn)
    redshift = halo_params["redshift"]
    mass = halo_params["mass"]
    rvir = halo_params["rvir"]
    rs = halo_params["rs"]

    try:
        rh3d_stars = halo_params['rh3D_physical_stars']
    except:
        rh3d_stars = stars.quant(1, 'kpc')

    if args.extra_radius is None:
        extra_radius = halo_params["rvir"]
    else:
        extra_radius = stars.quant(float(args.extra_radius[0]), str(args.extra_radius[1]))
        
    stars.refined_center6d(method="adaptative", nmin=nmin_stars)
    dm.refined_center6d(method="adaptative", nmin=nmin_dm)    
    if args.gas_cm == "darkmatter":
        gas.dq = {"cm": dm["cm"]}
        gas.dq = {"vcm": dm["vcm"]}
    elif args.gas_cm == "stars":
        gas.dq = {"cm": stars["cm"]}
        gas.dq = {"vcm": stars["vcm"]}
    else:
        gas.refined_center6d(method="rcc", rc_scale=rc_gas)

        
    stars_extra, dm_extra, gas_extra = stars.extend(extra_radius), dm.extend(extra_radius), gas.extend(extra_radius)

    print(f"Computing boundness...")
    ...
    stars_bound.refined_center6d(method="adaptative", nmin=nmin_stars)
    
    dm_bound = dm.filter(dm["total_energy"] < 0)
    dm_bound.refined_center6d(method="adaptative", nmin=nmin_dm)

    gas_bound = gas.filter(gas["total_energy"] < 0)
    #gas_bound.refined_center6d(method="adaptative", nmin=nmin_stars)

    
    result_dens = plot_voldens(axes[0, i], {"stars": stars_bound, "darkmatter": dm_bound, "gas": gas_bound}, {"stars": stars_extra, "darkmatter": dm_extra, "gas": gas_extra}, vol_components)
    bins = {
        "bound": { c: result_dens[c]["bound"]["bins"] for c in vol_components},
        "all": { c: result_dens[c]["all"]["bins"] for c in vol_components}
    }
    result_vels = plot_dispvel(axes[1, i], {"stars": stars_bound, "darkmatter": dm_bound, "gas": gas_bound}, {"stars": stars_extra, "darkmatter": dm_extra, "gas": gas_extra}, vol_components, bins)
    ...
    axes[1, i].text(2.1*soft, 0.9*pvmax, r"$M_*$="+f"{stars_bound['mass'].sum().value:.3e}"+r" $M_\odot$"+"\n"+r"$M_{dm}$="+f"{dm_bound['mass'].sum().value:.3e}"+r" $M_\odot$"+"\n"+r"$M_{gas}=$"+f"{gas_bound['mass'].sum().value:.3e}"+r" $M_\odot$" ,ha="left", va="top", color="black", rotation="horizontal", fontsize=14)
    ...
    if args.set_g is not None:
        ...
        fit_params_g = densModel.make_params(
            ...
            M={'value': stars_bound["mass"].sum().to("Msun").value, 'vary' : False},
            rh={'value': rh3d_stars.to('kpc').value, 'min': 1E-4, 'max': 50, 'vary': True},
    ...
    rho_h = mass / (4*np.pi/3 * rvir.to("kpc")**3)
    c = (rvir.to("kpc")/rs.to("kpc")).value
    r = np.logspace(np.log10(0.05), np.log10(300), 200)
    rho_nfw = NFWc(r, rho_h, c, rvir.to("kpc").value)
    

    fit_params_einasto = einastoModel.make_params(
        mu={'value': 2, 'min': 0.01, 'max': np.inf, 'vary': True},
        M={'value': dm_bound["mass"].sum().to("Msun").value, 'vary' : False},
        rs={'value': rs.to("kpc").value, 'min': 1E-4, 'max': 50, 'vary': True}
    )
    ...
    axes[0, i].plot(r, rho_nfw, color="darkorange", label=f'NFW Rockstar Fit: c={c:.2f}, rvir={rvir.to("kpc").value:.2f} kpc', zorder=9)
    ...
    result_surf = plot_surfdens(axes2[0, i], {"stars": stars_bound, "darkmatter": dm_bound, "gas": gas_bound}, {"stars": stars_extra, "darkmatter": dm_extra, "gas": gas_extra}, lines_of_sight, proj_components)
    bins = {
        "bound": { c: result_surf[c]["bound"]["bins"] for c in proj_components},
        "all": { c: result_surf[c]["all"]["bins"] for c in proj_components}
    }
    plot_losvel(axes2[1, i], {"stars": stars_bound, "darkmatter": dm_bound, "gas": gas_bound}, {"stars": stars_extra, "darkmatter": dm_extra, "gas": gas_extra}, lines_of_sight, proj_components, bins, velocity_projection)
    ...
    axes2[1, i].text(2.1*soft, 0.9*0.6*pvmax, r"$M_*$="+f"{stars_bound['mass'].sum().value:.3e}"+r" $M_\odot$"+"\n"+r"$M_{dm}$="+f"{dm_bound['mass'].sum().value:.3e}"+r" $M_\odot$"+r" $M_\odot$"+"\n"+r"$M_{gas}$="+f"{gas_bound['mass'].sum().value:.3e}"+r" $M_\odot$",ha="left", va="top", color="black", rotation="horizontal", fontsize=14)
