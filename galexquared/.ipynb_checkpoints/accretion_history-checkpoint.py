...
class AccretionHistory:
    ...
    def select_accreted_particles(self, subtree, ptype, z=None, t=None, indices=None, **kwargs):
        ...
        if not _check_particle_uniqueness(self.snap_particle_dict):
            print(f"Particles being born twice have been detected... Fixing that.")
    ...
    #     """Return a DataFrame of (particle_index, subtree, snapshot)
    #     giving each particle's subtree *at* snapshot=snap,
    #     based on the last delta event ≤ snap.
    #     """
    #     df = self.accreted_particles

    #     prior = df[df["Snapshot"] <= snap]
    #     if prior.empty:
    #         return pd.DataFrame(columns=["Snapshot","particle_index","Sub_tree_id"])

    #     last = (
    #         prior
    #         .sort_values(["particle_index","Snapshot"])
    #         .drop_duplicates(subset="particle_index", keep="last")
    #         .loc[:, ["particle_index","Time", "Sub_tree_id"]]
    #     )
    #     last["Snapshot"] = snap
    ...
    #     """Return the full event history for the given particle_index or list thereof.
    #     No NaN rows—only the delta events themselves.
    #     """
    #     df = self.accreted_particles
    #     track = df[df["particle_index"].isin(pid_list)].copy()
    #     if track.empty:
    #         return pd.DataFrame(columns=["Snapshot","particle_index","Sub_tree_id"])

    #     track = track.sort_values(["particle_index","Snapshot"])
    #     return track[["particle_index", "Snapshot", "Time", "Sub_tree_id"]]
    ...
    def _track_particle(self, pid_list):
        ...
        """
        mapping = self.equiv.set_index("Snapshot")["Time"]
